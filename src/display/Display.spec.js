...
describe('<Display />', () => {
  ...
  xtest('should display open and unlocked', () => {
    const { queryByText } = render(<Display />);
    expect(queryByText(/unlocked/i)).toBeTruthy();
    expect(queryByText(/open/i)).toBeTruthy();
  });
  xtest('should display closed if the closed prop is true', () => {
    const { queryByText } = render(<Display closed={true} />);
    expect(queryByText(/closed/i)).toBeTruthy();
  });
  xtest('should have red led as class when locked or closed', () => {
    const { queryAllByTestId } = render(
      <Display closed={true} locked={true} />
    );
    expect(queryAllByTestId('led red-led')).toBeTruthy();
  });
  xtest('should have green led as class when unlocked or open', () => {
    const { queryAllByTestId } = render(
